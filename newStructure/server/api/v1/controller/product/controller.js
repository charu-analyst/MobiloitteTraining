...
class product {
    ...
    // geting  product list using pagination.
    ...
    //product liked and disliked api with the use of push,pop,addtoset,addfield query.
    async productLikedAndDisliked(req, res, next) {
        ...
        try {
            ...
            if (like == true) {
                const alreadyliked = await product.productLikes.indexOf(req.userId);
                if (alreadyliked != -1) {
                    const popingId = await popProductLikeId(productId, req.userId);
                    return res.json(new successResponse(responseMessages.DISLIKED));
                } else {
                    const result = await addtoset(productId, req.userId);
                    return res.json(new successResponse(result, responseMessages.LIKED));
                }
            } else {
                return res.json(new successResponse(responseMessages.DISLIKED));
            }
    ...
    //product list and add fields of boolean type  using(addfields,aggraigation,aggrigatepopulate).
    async productlikedList(req, res, next) {
        try {
            const user = await findUser(req.userId);
            if (!user) {
                throw apiError(responseMessages.USER_NOT_FOUND);
            }
            const result = await addField(req.userId);
            if (!result) {
                throw apiError.notFound(responseMessages.NOT_LIKED_ANY_PRODUCT);
            }
            return res.json(new successResponse(result, responseMessages.SUCCESS));
        } catch (error) {
            console.log("Error", error);
